...
<?php
// Fallback etykiety, gdy 'name' jest puste:
if (!function_exists('schema_label_view')) {
	function schema_label_view(array $r): string {
		...
		if ($scheme === 'scheme1' || strtoupper($scheme) === 'S1') {
				$xa = $p['x_a'] ?? null;  $ya = $p['y_a'] ?? null;
				$xb = $p['x_b'] ?? null;  $yb = $p['y_b'] ?? null;
			...
			// S2: all≥y → topK (A [+ B])
			if ($scheme === 'scheme2' || strtoupper($scheme) === 'S2') {
				$ma = $p['min_all_a'] ?? null;  $ka = $p['top_k_a'] ?? null;
				$mb = $p['min_all_b'] ?? null;  $kb = $p['top_k_b'] ?? null;
				$a = ($ma!==null || $ka!==null) ? " A: all≥".($ma??'?').", topK=".($ka??'?') : '';
				$b = ($mb!==null || $kb!==null) ? " B: all≥".($mb??'?').", topK=".($kb??'?') : '';
				return trim('S2: all≥y → topK'.$a.$b);
			}
	...
	if ($hasB && (!empty($params['min_all_b']) || !empty($params['top_k_b']))) {
		$pTxt .= "; B: all≥".($params['min_all_b']??0).", topK=".($params['top_k_b']??0);
				...
				<tr data-id="<?= $s['id'] ?>" data-status="<?= esc($s['status']) ?>" data-next="<?= esc($s['current_from_draw'] ?? '') ?>">
					...
					<td><?= esc($s['name'] ?: schema_label_view($s)) ?></td>
